...
vi.mock('@/infra/repositories/domine-read.repository', () => ({
  domineReadRepository: { getOrder: vi.fn() },
}));
...
const request = (tenantId: string, orderId: string, authorization?: string) => {
  const headers = authorization ? { authorization } : undefined;
  return new NextRequest(`http://localhost/api/tenants/${tenantId}/domine/orders/${orderId}`, { headers });
};

beforeEach(() => {
  vi.clearAllMocks();
  vi.mocked(requireSessionTenantMatch).mockResolvedValue({
    ok: true,
    tenantId: 'tenant-a',
    sessionUser: { role: 'admin', sub: 'user-a' },
  });
});
...
describe('Domine order tenant isolation regression', () => {
  ...
  it('tenant B + order owned by tenant A must not expose tenant A data', async () => {
    vi.mocked(requireSessionTenantMatch).mockResolvedValue({
      ok: false,
