...
/**
 * POST /api/webhooks/whatsapp/status (canônico)
 * Compat: POST /api/whatsapp/status delega para este handler.
 */
    ...
    try {
        ...
        if (!contentType.includes('application/x-www-form-urlencoded')) {
            structuredLogger.warn('whatsapp_status_invalid_content_type', {
                route: '/api/webhooks/whatsapp/status',
                contentType,
            });
            return xmlAck();
        }
        ...
        if (process.env.NODE_ENV === 'production' && !verifyTwilioSignature(request, rawBody, payload)) {
            logger.warn('twilio_status_callback_invalid_signature', { route: '/api/webhooks/whatsapp/status' });
            return xmlAck(401);
        }
