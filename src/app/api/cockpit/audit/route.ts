    ...
    try {
        ...
        if (isDevRuntime() && serializedEvents.length === 0) {
            ...
            let mockEvents = [
                ...
                { id: 'evt_2', tenantId: tenantId, type: 'ITEM_UPDATED', payload: { status: 'success', resource: 'Produto XPTO' }, createdAt: new Date(Date.now() - 3600000) },
                { id: 'evt_3', tenantId: tenantId, type: 'BILLING_FAILED', payload: { status: 'failure', resource: 'Fatura #111' }, createdAt: new Date(Date.now() - 7200000) },
                { id: 'evt_4', tenantId: 'condstore', type: 'USER_LOGIN', createdAt: new Date(Date.now() - 86400000), payload: { status: 'success', resource: 'Autenticação' } },
                { id: 'evt_5', tenantId: 'condstore', type: 'ITEM_UPDATED', createdAt: new Date(Date.now() - 172800000), payload: { status: 'success', resource: 'Produto ABC' } },
                { id: 'evt_6', tenantId: 'condstore', type: 'USER_LOGOUT', createdAt: new Date(Date.now() - 200000000), payload: { status: 'success', resource: 'Autenticação' } },
                { id: 'evt_7', tenantId: 'condstore', type: 'ITEM_CREATED', createdAt: new Date(Date.now() - 300000000), payload: { status: 'success', resource: 'Produto ZZZ' } },
                { id: 'evt_8', tenantId: tenantId, type: 'BILLING_FAILED', createdAt: new Date(Date.now() - 400000000), payload: { status: 'failure', resource: 'Fatura #112' } },
                { id: 'evt_9', tenantId: tenantId, type: 'USER_LOGIN', createdAt: new Date(Date.now() - 500000000), payload: { status: 'failure', resource: 'Autenticação' } },
                { id: 'evt_10', tenantId: 'condstore', type: 'ITEM_UPDATED', createdAt: new Date(Date.now() - 600000000), payload: { status: 'success', resource: 'Produto YY' } },
                { id: 'evt_11', tenantId: tenantId, type: 'USER_LOGIN', createdAt: new Date(Date.now() - 700000000), payload: { status: 'success', resource: 'Autenticação' } },
            ];
            ...
            if (typeFilters) mockEvents = mockEvents.filter(e => e.type === typeFilters);
            if (statusFilter) mockEvents = mockEvents.filter(e => e.payload.status === statusFilter);
            if (actorFilter) mockEvents = mockEvents.filter(e => e.tenantId.toLowerCase().includes(actorFilter.toLowerCase()));
            if (resourceFilter) mockEvents = mockEvents.filter(e => e.payload.resource?.toLowerCase().includes(resourceFilter.toLowerCase()));
            if (startDate) mockEvents = mockEvents.filter(e => new Date(e.createdAt).getTime() >= new Date(startDate).getTime());
            if (endDate) {
                const end = new Date(endDate);
                end.setHours(23, 59, 59, 999);
            ...
            serializedEvents = mockEvents.slice(startIdx, startIdx + limit);
        }
