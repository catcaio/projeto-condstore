            ...
            {STATUS_PIPELINE.map((step, i) => {
                ...
                return (
                    <li key={step} className="flex-1 flex flex-col items-center relative">
                        {/* Connector line */}
                        {!isLast && (
                            <div className={`absolute top-3 left-1/2 w-full h-0.5 transition-colors ${done ? 'bg-[hsl(var(--ui-success))]' : 'bg-[hsl(var(--ui-border))]'}`} />
                        )}
                        {/* Step icon */}
...
function ShipmentCard({ row }: { row: ShipmentRow }) {
    ...
    return (
        <div className={`rounded-xl border bg-[hsl(var(--ui-surface))] p-4 flex flex-col gap-3 transition-all hover:shadow-sm ${isException ? 'border-[hsl(var(--ui-danger)/0.3)]' : isCancelled ? 'border-[hsl(var(--ui-border))]' : 'border-[hsl(var(--ui-border))]'
            }`}>
            {/* Card header */}
            <div className="flex items-start justify-between gap-2">
                ...
                <div className="text-right shrink-0">
                    {row.shipmentPrice !== null && (
                        <p className="text-sm font-bold text-[hsl(var(--ui-success))]">R$ {row.shipmentPrice.toFixed(2)}</p>
                    )}
            ...
            {isException && (
                <div className="rounded-lg bg-[hsl(var(--ui-danger)/0.1)] border border-[hsl(var(--ui-danger)/0.2)] px-3 py-2 text-xs font-semibold text-[hsl(var(--ui-danger))]">
                    ⚠️ Exceção detectada neste envio
                </div>
            )}
            {isCancelled && (
                <div className="rounded-lg bg-[hsl(var(--ui-bg-muted))] border border-[hsl(var(--ui-border))] px-3 py-2 text-xs font-semibold text-[hsl(var(--ui-text-muted))]">
                    Envio cancelado
                </div>
            )}
