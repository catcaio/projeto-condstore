...
import { RetryButton } from '../../cockpit/domine/dlq/_components/retry-button';

export const metadata = {
    title: 'DLQ — Sistema',
    description: 'Dead Letter Queue — eventos que falharam no processamento',
};

export default async function SistemaDLQPage({ searchParams }: { searchParams: Promise<{ [key: string]: string | string[] | undefined }> }) {
    ...
    if (role !== 'admin') {
        return (
            <div className="flex flex-col items-center justify-center p-12 text-center">
                ...
                <p className="text-[hsl(var(--ui-text-muted))]">Você não tem permissão para acessar o DLQ Manager.</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold tracking-tight text-[hsl(var(--ui-text))]">Dead Letter Queue</h1>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left text-sm whitespace-nowrap">
                            ...
                            <tbody className="divide-y divide-[hsl(var(--ui-border))]">
                                {dlqEvents.length === 0 ? (
                                    <tr>
                                        <td colSpan={5} className="px-4 py-6 text-center text-[hsl(var(--ui-text-muted))]">
                                            Nenhum evento na DLQ. Tudo limpo!
                                        </td>
                                    </tr>
                                ) : (
                                    dlqEvents.map(evt => (
                                        <tr key={evt.id} className="hover:bg-[hsl(var(--ui-bg))] transition-colors">
                                            <td className="px-4 py-3">
                                                <div className="font-mono text-xs font-semibold">{evt.id.slice(0, 8)}...</div>
                                                <div className="text-[hsl(var(--ui-text-muted))] text-xs mt-0.5">{evt.source}</div>
                                            </td>
                                            <td className="px-4 py-3 font-medium text-[hsl(var(--ui-text))]">{evt.type}</td>
                                            <td className="px-4 py-3">
                                                <Badge variant="outline" className="text-red-500 border-red-500 mb-1">{evt.errorCode || 'UNKNOWN_ERROR'}</Badge>
                                                <div className="text-[hsl(var(--ui-text-muted))] text-xs truncate max-w-xs" title={evt.errorMessage || ''}>
                                                    {evt.errorMessage || 'Sem log de erro'}
                                                </div>
                                            </td>
                                            <td className="px-4 py-3 text-[hsl(var(--ui-text-muted))] text-xs">
            ...
            <div className="flex justify-between items-center text-sm text-[hsl(var(--ui-text-muted))]">
                <p>Mostrando {dlqEvents.length} de {totalCount}</p>

                <div className="flex gap-2">
                    {page > 1 && (
                        <a href={`/sistema/dlq?page=${page - 1}`} className="px-3 py-1 rounded inline-flex border border-[hsl(var(--ui-border))] bg-[hsl(var(--ui-surface))] hover:bg-[hsl(var(--ui-bg))] text-[hsl(var(--ui-text))]">
                            Anterior
                        </a>
                    )}
