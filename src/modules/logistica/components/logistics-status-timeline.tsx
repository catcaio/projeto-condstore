import type { LogisticsTimelineStep } from '../mock-data';

function getDotClasses(state: LogisticsTimelineStep['state']) {
    if (state === 'issue') {
...
export function LogisticsStatusTimeline({ items }: { items: LogisticsTimelineStep[] }) {
    return (
            ...
            {items.map((item, index) => (
                <div key={item.label} className="flex gap-4">
                    <div className="flex w-6 shrink-0 flex-col items-center">
                        ...
                        {index < items.length - 1 ? <div className="mt-2 h-full min-h-8 w-px bg-[hsl(var(--ui-border))]" /> : null}
                    </div>
                    <div className="pb-5">
                        ...
                        <p className="mt-1 text-sm text-[hsl(var(--ui-text-muted))]">
                            {item.state === 'completed'
                                ? 'Checkpoint concluido com rastreabilidade.'
                                : item.state === 'current'
                                  ? 'Etapa atual do fluxo logistico.'
                                  : item.state === 'issue'
                                    ? 'Excecao ativa com necessidade de intervencao.'
                                    : 'Checkpoint ainda nao executado.'}
                        </p>
                        {item.timestamp ? (
