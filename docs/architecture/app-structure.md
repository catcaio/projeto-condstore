...
| `(auth)` | `src/app/(auth)/` | Páginas de autenticação (login, signup) | Layout próprio (sem sidebar) | Nenhuma (pré-auth) |
...
> [!NOTE]
> `/login` e `/signup` foram migrados para o route group `(auth)` e não são mais standalone.

**Regra:** Não criar novas rotas standalone. Novas páginas autenticadas vão em `(app)/`, públicas em `(public)/`, auth em `(auth)/`.
