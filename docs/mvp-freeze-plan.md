...
| Cotação de frete | `src/modules/fulfillment/freight/**`, `src/app/api/cockpit/conversations/[id]/quotes/**`, `src/app/(app)/cockpit/freight/**` | Faz parte do fluxo principal de venda e atendimento. Cotação pública anônima (`src/app/api/public/cotacao/**`) removida do MVP (issue #396). |
...
4. `src/app/api/cockpit/conversations/[id]/quotes/[quoteId]/send/route.ts` publica eventos para DOMINE. O console DOMINE está frozen, mas o barramento continua sendo suporte do MVP. (A rota `src/app/api/public/cotacao/quotes/route.ts`, citada aqui anteriormente, foi removida do MVP na issue #396.)
