...
|---|---|---|---|---|
| ClickUp | Central executiva e de governança | Roadmap, produto, marketing, prioridades, governança, dashboard executivo, go/no-go | Backlog técnico duplicado em detalhe operacional de engenharia | Direção executiva, prioridade e decisão de negócio |
| Linear | Esteira técnica de engenharia | Execução técnica, bugs, PR-blocks, cycles, tarefas de agentes técnicos, status de engenharia | Documentação estratégica longa, memória institucional, governança executiva | Estado operacional de execução de engenharia |
| GitHub | Fonte técnica absoluta | Código, branches, commits, PRs, reviews, merge | Gestão executiva, priorização de roadmap, decisões estratégicas longas | Verdade técnica do software e histórico de mudanças |
...
| Jules | Agente executor técnico principal | Execução de PRs fechadas com escopo claro e critérios definidos | Decisão estratégica, merge autônomo, DONE final | Execução automatizada com evidência de implementação |
...
| Codex | Fallback técnico e executor documental versionado | Código, testes, correções pontuais e documentação no repositório | Autoridade de DONE final, decisão estratégica sem owner | Entrega técnica/documental versionada |
...
- **Jules**: executor principal de PRs fechadas.
- **Antigravity**: auditor técnico / executor com MCP, somente se bridge estiver validado.
- **Codex**: fallback técnico para código, testes e correções pontuais; executor documental quando a entrega for arquivo versionado no repositório.
